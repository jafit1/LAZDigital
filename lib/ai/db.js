// lib/ai/db.js — lapisan basis data modul AI Asisten
//
// Sumber kebenaran: Upstash Redis (REST). Bila variabel Upstash kosong,
// otomatis jatuh ke penyimpanan lokal berkas .data/ai.json supaya
...
// Namespace TERPISAH dari laz:*, blast:*, dan fund:* — modul AI berbagi basis
// data Upstash yang sama tetapi tidak boleh berbenturan kuncinya.
//
// DI SINILAH KUNCI API PROVIDER DISIMPAN (ai:penyedia:*). Karena itu berkas ini
// tidak pernah mengirim isinya apa adanya ke tampilan: penyamaran dikerjakan di
// lib/ai/penyedia.js, dan repo ini publik sehingga kunci tidak boleh pernah
// masuk git.
const PREFIKS = 'ai:';
...
if (DI_VERCEL && !PAKAI_UPSTASH) {
  console.error(
    ...
    'Di Vercel, modul AI wajib memakai Upstash Redis — penyimpanan lokal tidak tersedia.'
  );
}
...
// ------------------------------------------------------------------
const BERKAS_LOKAL = path.join(process.cwd(), '.data', 'ai.json');
let lokal = null;
