...
const BENTUK = {
  openai: {
    ...
    contohModelLain: 'gpt-4o, gpt-4o-mini, o4-mini',
  },
  anthropic: {
    ...
    contohModelLain: 'claude-opus-4-20250514, claude-haiku-4-20250514',
  },
  gemini: {
    label: 'Google Gemini',
    ...
    contohModelLain: 'gemini-2.0-flash-lite, gemini-2.5-pro',
  },
};
...
/* Diterima sebagai teks dipisah koma ATAU sebagai array — tampilan mengirim
   teks, uji dan impor mengirim array. Yang keluar selalu array bersih tanpa
   duplikat dan tanpa model utama. */
function daftarModelLain(nilai) {
  const mentah = Array.isArray(nilai) ? nilai : String(nilai || '').split(/[,\n]/);
  const bersih = [];
  for (const m of mentah) {
    const t = bersihkanTeks(m, 80).trim();
    if (t && !bersih.includes(t)) bersih.push(t);
    if (bersih.length >= 12) break;
  }
  return bersih;
}

/* Semua model yang boleh dipilih, sudah rata dan siap digambar jadi daftar
   pilihan. Disusun di server, bukan di tampilan: kalau tampilan yang
   menyusunnya, aturan "model apa yang sah" akan hidup di dua tempat. */
async function daftarModel() {
  const aktif = await db.ambil(AKTIF);
  const keluar = [];
  for (const p of (await semua()).sort((a, b) => String(a.nama).localeCompare(String(b.nama), 'id'))) {
    for (const m of [p.model, ...(p.modelLain || [])]) {
      if (!m) continue;
      keluar.push({
        penyediaId: p.id,
        penyediaNama: p.nama,
        ...
        model: m,
        utama: m === p.model,
        dukungGambar: p.dukungGambar !== false,
        siap: Boolean(p.kunci) || /localhost|127\.0\.0\.1/.test(p.url),
        baku: p.id === aktif && m === p.model,
      });
    }
  }
  return keluar;
...
/* Model yang diminta tampilan HARUS ada di daftar provider itu. Tanpa
   pemeriksaan ini, siapa pun yang bisa membuka chat dapat mengetikkan nama
   model apa pun — termasuk yang jauh lebih mahal per token — lewat permintaan
   yang dibuat sendiri. */
function modelSah(p, m) {
  if (!p || !m) return false;
  return m === p.model || (p.modelLain || []).includes(m);
}
...
async function simpan(data, pengguna) {
  ...
  const p = {
    ...
    /* Model lain yang boleh dipilih dari kotak chat. Ditulis superadmin, bukan
       diambil dari daftar model provider: daftar itu berbeda-beda bentuknya di
       tiap layanan, berubah tanpa pemberitahuan, dan sebagian besar isinya
       tidak pernah dipakai lembaga ini. */
    modelLain: daftarModelLain(data.modelLain !== undefined ? data.modelLain : (lama ? lama.modelLain : [])),
    dukungGambar: data.dukungGambar === undefined
      ? (lama ? lama.dukungGambar !== false : true) : Boolean(data.dukungGambar),
...
async function aliran(p, { sistem, pesan, suhu, maksToken, sinyal, model }, onToken) {
  const t = {
  ...
  /* Model yang tidak sah DIABAIKAN diam-diam ke model utama, bukan ditolak
     dengan galat: satu-satunya cara nilai itu bisa tidak sah adalah bila
     provider diubah setelah percakapan dimulai, dan menggagalkan pertanyaan
     orang karena itu tidak menolong siapa pun. */
  const pakaiModel = modelSah(p, model) ? model : p.model;
  return t({ ...p, model: pakaiModel }, {
...
/* Gambar dikirim berbeda-beda bentuknya oleh ketiga layanan, jadi tiap adapter
   menyusunnya sendiri. Yang dipakai bersama hanya bentuk masukannya:
   pesan.gambar = [{mime, data(base64), nama}]. */
const adaGambar = (m) => Array.isArray(m.gambar) && m.gambar.length > 0;

async function aliranOpenAI(p, o, onToken) {
  const pesan = [];
  if (o.sistem) pesan.push({ role: 'system', content: o.sistem });
  for (const m of o.pesan) {
    const peran = m.peran === 'assistant' ? 'assistant' : 'user';
    if (!adaGambar(m)) { pesan.push({ role: peran, content: m.isi }); continue; }
    pesan.push({
      role: peran,
      content: [
        { type: 'text', text: m.isi },
        ...m.gambar.map((g) => ({
          type: 'image_url',
          image_url: { url: `data:${g.mime};base64,${g.data}` },
        })),
      ],
    });
  }
...
async function aliranAnthropic(p, o, onToken) {
  const res = await fetch(`${p.url}/v1/messages`, {
    ...
    body: JSON.stringify({
      ...
      messages: o.pesan.map((m) => ({
        role: m.peran === 'assistant' ? 'assistant' : 'user',
        content: adaGambar(m)
          ? [
            ...m.gambar.map((g) => ({
              type: 'image',
              source: { type: 'base64', media_type: g.mime, data: g.data },
            })),
            { type: 'text', text: m.isi },
          ]
          : m.isi,
...
module.exports = {
  ...
  daftarModel, modelSah, daftarModelLain,
