...
async function buat({ judul, penyediaId, personaId }, pengguna) {
  const s = {
    id: id('s_'),
...
async function tambahPesan(sesiId, { peran, isi, olehNama, olehId, model, penyedia, token, terpotong }) {
  ...
  if ((!s.judul || s.judul === 'Percakapan baru') && p.peran === 'user') s.judul = judulDari(p.isi);
  s.diubah = sekarang();
  await db.simpan(KUNCI(s.id), s);
...
async function daftar({ cari = '' } = {}) {
  ...
  if (cari) {
    ...
    isi = isi.filter((s) => String(s.judul).toLowerCase().includes(q)
      || (s.pesan || []).some((p) => String(p.isi).toLowerCase().includes(q)));
  }
