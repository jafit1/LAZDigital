...
const lampiranLib = require('./lampiran');
...
/* Riwayat untuk provider: paling baru yang dipertahankan, dipotong dari yang
   ...
   Lampiran ikut dibentangkan di sini: hasil bacaan berkas disambungkan ke teks
   pesannya, sedangkan GAMBAR dibawa terpisah karena tiap provider menyusunnya
   dengan bentuk yang berbeda. */
async function riwayatUntukModel(pesan, { bolehGambar = true } = {}) {
  const ambil = (pesan || []).slice(-MAKS_RIWAYAT);
  const keluar = [];
  let huruf = 0;
  let gambarTerpakai = 0;

  for (let i = ambil.length - 1; i >= 0; i--) {
    const m = ambil[i];
    let isi = String(m.isi || '');
    const gambar = [];

    if ((m.lampiran || []).length) {
      const bagian = await lampiranLib.untukModel(m.lampiran);
      for (const b of bagian) {
        if (b.jenis === 'gambar') {
          /* Gambar lama TIDAK ikut dikirim ulang selamanya. Tiap gambar
             berharga ratusan sampai ribuan token pada SETIAP giliran; sebuah
             percakapan panjang berisi foto akan membayar ulang seluruhnya
             setiap kali ada yang bertanya. Yang terbaru saja yang dibawa. */
          if (!bolehGambar || gambarTerpakai >= MAKS_GAMBAR_RIWAYAT) {
            isi += `\n[Gambar "${b.nama}" tidak disertakan pada giliran ini.]`;
            continue;
          }
          gambarTerpakai++;
          gambar.push(b);
    ...
    if (!isi.trim() && !gambar.length) continue;
    if (huruf + isi.length > MAKS_HURUF_RIWAYAT && keluar.length) break;
    huruf += isi.length;
    keluar.unshift({
      peran: m.peran === 'assistant' ? 'assistant' : 'user',
      isi: isi.trim() || '(berkas terlampir)',
      gambar,
    });
  }
...
async function alirkan(req, res, { badan, pengguna }) {
  ...
  try {
    ...
    const berkasMasuk = Array.isArray(badan.lampiran) ? badan.lampiran : [];
    if (!isiPesan && !berkasMasuk.length) throw new GalatAplikasi('Pertanyaan masih kosong.', 400);
    ...
    if (!s) {
      s = await percakapan.buat({
        penyediaId: badan.penyediaId, personaId: badan.personaId, model: badan.model,
      }, pengguna);
    }
    ...
    const modelDiminta = badan.model || s.model || '';
    const modelDipakai = penyediaLib.modelSah(penyedia, modelDiminta) ? modelDiminta : penyedia.model;
    const bolehGambar = penyedia.dukungGambar !== false;
    ...
    /* Berkas disimpan SEBELUM pertanyaannya, supaya metadata yang masuk ke
       riwayat sudah punya id yang benar. Kalau penyimpanannya gagal (berkas
       kelewat besar, jenisnya tidak didukung), galatnya keluar di sini —
       sebelum apa pun tercatat dan sebelum provider dihubungi. */
    const berkas = await lampiranLib.simpanBanyak(berkasMasuk, pengguna);
    if (berkas.some((b) => b.jenis === 'gambar') && !bolehGambar) {
      throw new GalatAplikasi(
        `Model "${modelDipakai}" pada provider "${penyedia.nama}" tidak disetel bisa membaca gambar. `
        + 'Pilih model lain di kotak chat, atau minta superadmin mencentang "bisa membaca gambar" pada provider itu.', 400);
    }
    ...
    const simpanTanya = await percakapan.tambahPesan(s.id, {
      ...
      lampiran: lampiranLib.untukSimpan(berkas),
    });
    s = simpanTanya.sesi;

    /* Pilihan model disimpan di sesinya supaya percakapan yang dilanjutkan
       orang lain memakai model yang sama, bukan kembali ke model baku. */
    if (modelDipakai !== s.model) s = await percakapan.ubah(s.id, { model: modelDipakai });

    const riwayat = await riwayatUntukModel(s.pesan, { bolehGambar });
    ...
    kirim({
      ...
      penyedia: { nama: penyedia.nama, model: modelDipakai, bentuk: penyedia.bentuk },
      lampiran: lampiranLib.untukTampilan(berkas),
      otak: { ...sistem.rincian, gambar: riwayat.reduce((n, r) => n + (r.gambar || []).length, 0) },
    });
    ...
    if (jawaban.trim()) {
      const simpanJawab = await percakapan.tambahPesan(s.id, {
        ...
        model: modelDipakai, penyedia: penyedia.nama,
        token: token || null,
...
module.exports = {
  alirkan, susunSistem, riwayatUntukModel,
  IDENTITAS, MAKS_RIWAYAT, MAKS_HURUF_RIWAYAT, MAKS_GAMBAR_RIWAYAT, DETIK_DENYUT,
};
