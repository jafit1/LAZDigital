// lib/fund/db.js — lapisan basis data modul Fundraising
//
// Sumber kebenaran: Upstash Redis (REST). Bila variabel Upstash kosong,
// otomatis jatuh ke penyimpanan lokal berkas .data/fund.json supaya
...
// Namespace TERPISAH dari blast:* dan laz:* — modul Fundraising berbagi
// basis data Upstash yang sama tetapi tidak boleh berbenturan kuncinya.
// Berkas ini sengaja saudara mandiri dari lib/blast/db.js: menyatukannya
// berarti menyentuh Broadcast yang sudah teruji hanya demi menghemat 200 baris.
const PREFIKS = 'fund:';
...
if (DI_VERCEL && !PAKAI_UPSTASH) {
  console.error(
    ...
    'Di Vercel, modul Fundraising wajib memakai Upstash Redis — penyimpanan lokal tidak tersedia.'
  );
}
...
// ------------------------------------------------------------------
const BERKAS_LOKAL = path.join(process.cwd(), '.data', 'fund.json');
let lokal = null;
