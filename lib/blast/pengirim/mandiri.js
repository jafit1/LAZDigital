...
async function periksa(perangkat) {
  ...
  return {
    ...
    nomor: kabar.nomor || '',
    /* QR ikut dibawa di sini supaya tampilan cukup MENANYAKAN keadaan sambil
       menunggu, bukan mengirim perintah "sambungkan" berulang-ulang. Perintah
       yang diulang membuat gateway membuka sambungan baru setiap kali, dan
       setiap sambungan baru membatalkan QR yang sedang dipindai — itulah yang
       bikin pemindaian tidak pernah selesai. */
    qr: kabar.qr || '',
    keterangan: kabar.keterangan || 'Gateway terhubung.',
  };
}

/* Mengganti nomor WhatsApp perangkat ini.
   Bukan sekadar memutus: sesi lama harus benar-benar dihapus di gateway. Kalau
   hanya diputus, penyambungan berikutnya memakai sesi yang tersimpan dan
   kembali masuk ke nomor yang LAMA — tidak akan pernah muncul QR, dan terlihat
   seperti penggantian nomornya gagal tanpa sebab. */
async function gantiNomor(perangkat) {
  await db.simpan(KUNCI_PERINTAH(perangkat.id), { perintah: 'ganti-nomor', waktu: sekarang() }, { detik: 600 });
  await db.hapus(KUNCI_AGEN(perangkat.id));
  return {
    status: 'menunggu',
    keterangan: 'Sesi lama sedang dikeluarkan di gateway. QR untuk nomor baru akan muncul sebentar lagi.',
  };
}

module.exports = {
  nama, label, butuhKredensial,
  kirim, sambungkan, putuskan, periksa, gantiNomor,
