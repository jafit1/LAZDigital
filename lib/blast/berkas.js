...
/* Lampiran yang menempel pada TEMPLAT tidak boleh ikut aturan tujuh hari.
   Templat dipakai berbulan-bulan — brosur zakat fitrah dipanggil lagi tiap
   Ramadan — dan lampiran yang diam-diam menghilang menghasilkan kegagalan yang
   paling membingungkan: templatnya masih ada, isinya masih benar, tetapi
   berkasnya tidak pernah sampai.

   Mengembalikan false bila berkasnya sudah telanjur hilang, supaya pemanggilnya
   bisa mengatakan "unggah ulang" alih-alih menyimpan templat yang cacat. */
async function jadikanAbadi(berkasId) {
  if (!berkasId) return false;
  const keterangan = await db.ambil(KUNCI(berkasId));
  if (!keterangan) return false;
  const isi = await db.ambil(KUNCI_ISI(berkasId));
  if (!isi) return false;
  keterangan.abadi = true;
  await db.simpan(KUNCI_ISI(berkasId), isi);     // tanpa opsi detik = tanpa umur
  await db.simpan(KUNCI(berkasId), keterangan);
  return true;
}
...
module.exports = { simpanBerkas, jadikanAbadi, ambilKeterangan, ambilIsi, JENIS, BATAS_BYTE, UMUR_DETIK, KUNCI, KUNCI_ISI };
