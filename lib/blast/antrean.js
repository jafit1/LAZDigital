...
async function antrikan(data) {
  ...
  const pesan = {
    ...
    isi: {
      ...
      /* Lampiran disebut lewat id, bukan disalin isinya ke tiap pesan: satu PDF
         untuk lima ratus penerima cukup disimpan sekali. Tanpa tiga baris ini
         id-nya hilang di sini dan lampirannya tidak pernah sampai ke gateway. */
      berkasId: (data.isi && data.isi.berkasId) || '',
      tipeBerkas: (data.isi && data.isi.tipeBerkas) || '',
      jenisBerkas: (data.isi && data.isi.jenisBerkas) || '',
    },
    arah: 'keluar',
    status: 'antre',
...
/* --- Menghapus riwayat -----------------------------------------------------
 *
 * Satu pesan tercatat di BANYAK tempat sekaligus: dokumennya sendiri, daftar
 * pesan terbaru, himpunan antrean, kotak surat mati, himpunan serahan, antrean
 * gateway per perangkat, dan utas percakapan per nomor. Menghapus dokumennya
 * saja meninggalkan id yatim di enam tempat lain — dan yang paling berbahaya
 * dari enam itu adalah antrean: id yang masih terdaftar di sana akan terus
 * dicoba tiap putaran, jadi "pesan yang dihapus" justru bisa tetap terkirim.
 *
 * Yang TIDAK ikut dihapus: catatan audit. Audit ada justru untuk mencatat siapa
 * menghapus apa; mengosongkannya bersama pesannya menghapus jejak itu sendiri.
 */
async function hapusPesan(pesanId) {
  ...
  await db.keluarDariHimpunan('pesan:gagal', pesan.id);
  if (pesan.perangkatId) await db.keluarDariHimpunan(`agen:keluar:${pesan.perangkatId}`, pesan.id);
  if (pesan.nomor) await db.keluarDariHimpunan(`percakapan:${pesan.nomor}`, pesan.id);

  const daftar = (await db.ambil('pesan:baru')) || [];
  const sisa = daftar.filter((i) => i !== pesan.id);
  if (sisa.length !== daftar.length) await db.simpan('pesan:baru', sisa);

  /* Kunci idempoten dilepas juga. Kalau ditinggalkan, mengirim ulang kiriman
     massal yang sama akan dianggap kembar dan diam-diam tidak jadi dikirim —
     padahal riwayatnya sudah tidak ada, jadi tidak ada yang bisa dilihat untuk
     mengerti kenapa. */
  if (pesan.kunciIdempoten) await db.hapus(`idem:${pesan.kunciIdempoten}`);
  await db.hapus(KUNCI_PESAN(pesan.id));
  return pesan;
}

/* Mengosongkan seluruh riwayat pesan. Dipakai sesudah masa uji coba, saat
   nomor pribadi dipakai untuk mencoba-coba dan isinya tidak layak jadi arsip
   lembaga. Kiriman massal dan kontak TIDAK disentuh. */
async function hapusSemuaPesan() {
  const daftar = (await db.ambil('pesan:baru')) || [];
  const idAntre = await db.anggotaHimpunan(KUNCI_ANTREAN);
  const idSerahan = await db.anggotaHimpunan(KUNCI_SERAHAN);
  const idGagal = await db.anggotaHimpunan('pesan:gagal');
  const semua = Array.from(new Set([...daftar, ...idAntre, ...idSerahan, ...idGagal]));

  let terhapus = 0;
  for (const i of semua) {
    try { await hapusPesan(i); terhapus++; } catch (_) { /* sudah hilang duluan */ }
  }

  /* Dibereskan terakhir dan tanpa syarat: kalau ada id yang dokumennya sudah
     hilang lebih dulu, hapusPesan di atas melewatinya dan id itu akan tertinggal
     di himpunan — lalu tiap putaran antrean menyapunya lagi sebagai "yatim". */
  await db.hapus('pesan:baru');
  for (const kunci of [KUNCI_ANTREAN, KUNCI_SERAHAN, 'pesan:gagal']) {
    const sisa = await db.anggotaHimpunan(kunci);
    if (sisa.length) await db.keluarDariHimpunan(kunci, ...sisa);
  }
  return { terhapus, diperiksa: semua.length };
}
